...
document.addEventListener("DOMContentLoaded", function () {
  // সব .nav-item.dropdown-এ hover behavior যোগ করবো (large screens এ)
  document.querySelectorAll(".nav-item.dropdown").forEach(function (dropdown) {
    var toggle = dropdown.querySelector('[data-bs-toggle="dropdown"]');
    if (!toggle) return;

    // mouseenter -> show dropdown (only on wide screens)
    dropdown.addEventListener("mouseenter", function () {
      if (window.innerWidth > 992) {
        // large screens এ hover চালু রাখছি
        bootstrap.Dropdown.getOrCreateInstance(toggle).show();
      }
    });

    // mouseleave -> hide dropdown
    ...
    // mobile/touch এ dropdown টগল বোতামে ক্লিক করলেই কাজ করবে (default)
    // আর parent link আলাদা রয়েছে, তাই ক্লিক করলে নেভিগেট করবে।
  });
});
